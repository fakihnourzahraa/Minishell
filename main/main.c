...
int main(int argc, char **argv, char **envp)
{
    ...
    while (!shell.exit)
    {
        ...
        // Handle signal BEFORE processing command (only handle if not in heredoc)
        if (g_signal == SIGINT && !shell.in_h)
        {
            g_signal = 0;
        ...
        // Reset any stray signals from child processes
        if (g_signal == SIGINT)
            g_signal = 0;
...
// FIXED: External global variable declaration
extern int g_signal;
...
int main(int argc, char **argv, char **envp)
{
    ...
    while (!shell.exit)
    {
        ...
        // Reset signal flag before execution
        g_signal = 0;
