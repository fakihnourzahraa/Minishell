...
/*   Updated: 2025/08/31 19:35:02 by nfakih           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//let val = the designated function, and if it fails or 
//smthg goes wrong let it = 0, else the length of the word , then handle the rest in is cmd function
...
int	is_op(char	*a, int i)
{
	...
	s[0] = a[i++];
	if (a[i] == '<' || a[i] == '>' 
		|| ft_strnstr(a, s, "<<") || ft_strnstr(a, s, ">>"))0```````````````
	return (i);
}
